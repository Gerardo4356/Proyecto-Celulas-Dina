        ...
        for i, point in enumerate(points):
            ...
            if result.multi_hand_landmarks:
                ...
                if abs(x - point[0]) < 20 and abs(y - point[1]) < 20:
                    ...
                    elif points_touch_time[i] == 0:
                        points_touch_time[i] = time.time()
                    elif time.time() - points_touch_time[i] > 1:  # Si ha pasado más de 1 segundos
                        if not pygame.mixer.music.get_busy():
                            pygame.mixer.music.load(f'{i+1}.mp3')
                            pygame.mixer.music.play()
                            points_touch_time[i] = 0  # reseteamos el tiempo para el siguiente evento
                else:
                    points_touch_time[i] = 0  # si la mano no está sobre el punto, reseteamos el tiempo
